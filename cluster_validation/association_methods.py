...
class AssociationMethods(object):
    ...
    def search_distance(truth_data, cluster_data, r_max, r_max_type, cosmo, association_way, delta_ovd=200):
        ...
        if r_max_type=="fixed_angle":
            ...
            if association_way=="1w":
                r_max_scaled = AssociationMethods.r_scaled(truth_data['halo_mass'], truth_data['redshift'], delta_ovd, cosmo, density_type="critical")
            elif association_way=="2w":
                r_max_scaled = AssociationMethods.r_richness_scaled(cluster_data['richness'])                       
    ...
    @staticmethod
    def select_one_association(cat_ref, number_of_volume_match, list_of_volume_match, method, coo_ref=None, coo_cat=None, 
                               cat=None, truth_member_data=None, cluster_member_data=None, association_way=None):

        ind_cat_ref_match = []
        ind_cat_match     = [] 

        if method=='nearest':
            angular_distances_of_associations = AssociationMethods.get_angular_distances(cat_ref, list_of_volume_match, coo_ref, coo_cat)
        elif method=='membership':   
            membership_of_associations = AssociationMethods.get_membership(cat_ref, cat, list_of_volume_match, truth_member_data, 
                                                                           cluster_member_data, association_way)
    ...
    def volume_match(truth_data, cluster_data, delta_zmax, r_max, r_max_type, method, cosmo, truth_member_data=None, cluster_member_data=None, delta_ovd=200, density_type="critical"):
        ...
        theta_max = AssociationMethods.search_distance(truth_data, cluster_data,r_max, r_max_type, cosmo, "1w", delta_ovd=delta_ovd)

        ###search and store volume candidate------------------------------------------------------
        number_of_volume_match_1w, list_of_volume_match_1w = \
        AssociationMethods.volume_associations(truth_data, cluster_data, theta_max, delta_zmax, "1w", coo_DC2, coo_RM)

        ###select only one match---------------------------------------------------------------------
        ind_DC2_match_1w, ind_RM_match_1w = \
        AssociationMethods.select_one_association(truth_data, number_of_volume_match_1w, list_of_volume_match_1w, method, coo_DC2, 
                                                  coo_RM, cluster_data, truth_member_data, cluster_member_data, "1w")

        ########################################################################################
        #one way association : RM => DC2 (2w)

        ###compute searching distances-------------------------------------------------------------
        theta_max = AssociationMethods.search_distance(truth_data, cluster_data,r_max, r_max_type, cosmo, "2w")

        ###search and store volume candidate------------------------------------------------------
        number_of_volume_match_2w, list_of_volume_match_2w = \
        AssociationMethods.volume_associations(truth_data, cluster_data, theta_max, delta_zmax, "2w", coo_DC2, coo_RM)

        ###select only one match---------------------------------------------------------------------
        ind_RM_match_2w, ind_DC2_match_2w = \
        AssociationMethods.select_one_association(cluster_data, number_of_volume_match_2w, list_of_volume_match_2w, method, coo_RM, coo_DC2, 
                                                  truth_data, truth_member_data, cluster_member_data, "2w")

        ########################################################################################       
        #bijective associations

        ind_bij = AssociationMethods.bijective_associations(ind_RM_match_1w, ind_DC2_match_1w, ind_RM_match_2w, ind_DC2_match_2w)
